...
1.luego de crearlo lo importamos en el app-routing.module.ts 
import { myGuardLoginGuard } from './guards/my-guard-login.guard';

2. luego lo ponemos con el componente que queremos proteger, ejemplo:

{path: 'compo1', component: Compo1Component, canActivate:[myGuardLoginGuard]},

|--------------------------------------------------------------------------
| Utilizar Http service
|--------------------------------------------------------------------------

1:Importar el modulo HttpClientModule en app.module.ts i ponerlo en los imports:
...
| Interceptors
|--------------------------------------------------------------------------
| se encarga de adjuntar el token de acceso JWT a todas las solicitudes salientes antes de enviarlas al servidor


1.despues de crearla i configurarla hay que importarla en el app.module.ts
import { JwtInterceptor } from './jwt.interceptor'; // Importa tu interceptor aquí

2.i despues añadirla en el apartado de providers, Ejemplo:
