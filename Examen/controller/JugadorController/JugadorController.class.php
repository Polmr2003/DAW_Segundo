...
class JugadorController implements ControllerInterface
{
    ...
    public function ejer1()
    {
        ...
        $this->view->display("view/Ejer1/Ejercicio1.php");
    }
    ...
    {
        //validem i omplim missatges d'error, si n'hi hagués
        $jugadorValid = JugadorFormValidation::checkData(JugadorFormValidation::MODIFY_FIELDS);
        ...
        if (empty($_SESSION['error'])) {
            ...
            $getName = $jugadorValid->getNombre();

            if ($searchID) {
                $product = [$searchID, $getName];

                $this->model->modify($product);
                $_SESSION['info'] = JugadorMessage::INF_FORM['update'];
            } else {
